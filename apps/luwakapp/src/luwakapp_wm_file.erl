...
%%      query parameter.
service_available(RD, #ctx{ip=IP, port=Port}=Ctx) ->
    case riak_connection(IP, Port) of
        {ok, C} ->
...
produce_toplevel_body(RD, Ctx=#ctx{client=C}) ->
    ...
    KeyPart =
        case wrq:get_qs_value(?Q_KEYS, RD) of
            ?Q_STREAM -> stream;
            ?Q_TRUE   ->
                {ok, KeyList} = riakc_pb_socket:list_keys(C, ?O_BUCKET),
                [{?Q_KEYS, KeyList}];
            _         -> []
        end,
    case KeyPart of
        stream -> {{stream, {mochijson2:encode({struct, SchemaPart}),
                             fun() ->
                                     {ok, ReqId} = 
                                         riakc_pb_socket:stream_list_keys(
                                           C, ?O_BUCKET),
                                     stream_keys(ReqId)
                             end}},
                   RD,
...
%% stolen from riak_core_util:unique_id_62/0
random_key() ->
    Rand = crypto:sha(term_to_binary({make_ref(), now()})),
    <<I:160/integer>> = Rand,
    integer_to_list(I, 62).

riak_connection(IP, Port) ->
    case get({riakc, IP, Port}) of
        undefined ->
